...
\begin{center} % Everything within the center environment is centered.
	...
	{\studentName\ } % <---- Fill in your names here!
...
The complete code cannot be displayed because of limited report length, and the main missing part is the dataset.
For that, the general idea is to compose a \mintinline{text}{TensorDataset} in \mintinline{text}{PyTorch} with the data, after loading and shuffling data.
...
\begin{figure}[H]
	...
	\caption{Add captions here}
\end{figure}
From the figure above, we can see the training loss and validation loss (noted as Test Loss in the figure) decreases as the training process goes on.
Concretely, this neural network achieves a best validation loss at 0.76, which is slightly lower than the collaborative filter implementation.
Therefore, for this movie recommender system, we would (slightly) prefer the Embedding net solution.
